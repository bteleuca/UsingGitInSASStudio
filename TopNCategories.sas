...
proc means data=&data &stat noprint;
  ...
  output out=sasdm.summary &stat=&measure &category /levels;
run;
...
from sasdm.summary where _TYPE_=0;
select count(distinct &category) into :categorycount from sasdm.summary;
quit;

/* sort the results so that we get the TOP values */
/* rising to the top of the data set */
proc sort data=sasdm.summary out=sasdm.topn;
  where _type_>2;
